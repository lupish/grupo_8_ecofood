...
const controller = {
    ...
    listProducts: async (req, res) => {
        try {
        let lista = await Producto.findAll({include:[{association: 'ProductoImagen'}, {association: 'Marca'}, {association: 'EstiloVida'}]}, {where: {EstiloVida:{id: {[Op.eq]:[req.params.idEstiloVida]}}}})
        if(req.params.idEstiloVida){
            lista
            res.render('products/listProducts', {prods: Producto})
        }else{
            lista = await Producto.findAll({include:[{association: 'ProductoImagen'}, {association: 'Marca'}, {association: 'EstiloVida'}]});
        }
        }
        catch (error){
            console.log(error);
